...
export class YouTubeTranscriptService {
  ...
  private async extractTranscript(videoId: string): Promise<YouTubeTranscript | null> {
    try {
      // Use youtube-transcript library or similar
      ...
      return {
        text: transcript.map((segment: any) => segment.text).join(' '),
        segments: transcript.map((segment: any) => ({
          text: segment.text,
          start: segment.start,
          duration: segment.duration,
        })),
        language: 'en', // YouTube API would provide this
        isAutoGenerated: true, // Would check if manual or auto-generated
      };
    } catch (error) {
